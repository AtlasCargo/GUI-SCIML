...
# (Assumes linear_model and sse_objective from Slide 5 are defined)
# (Assumes t_data, y_data are defined)

theta_0_range = -1.0:0.1:3.0
theta_1_range = 0.5:0.1:3.5 # Adjusted range based on previous example

sse_surface = zeros(length(theta_0_range), length(theta_1_range))

for (i, th0) in enumerate(theta_0_range)
    for (j, th1) in enumerate(theta_1_range)
        sse_surface[i, j] = sse_objective([th0, th1], t_data, y_data)
    end
end

# Contour plot
cp = contour(theta_1_range, theta_0_range, sse_surface, # Note: x-axis (theta_1), y-axis (theta_0)
        fill=true, levels=20, color=:viridis,
        xlabel="Parameter theta_1", ylabel="Parameter theta_0",
        title="SSE Contour Plot for [theta_0, theta_1]",
        aspect_ratio=:equal) # Try to make axes scales similar if appropriate
scatter!([2.0], [1.0], label="Approx. True Parameters", color=:red, markersize=5, markerstrokecolor=:white)
# savefig("sse_contour_2D.png")

display(cp)
println("2D SSE contour plot generated (conceptually).")

# (Optional: Surface plot - can be harder to interpret for minima)
# surface(theta_1_range, theta_0_range, sse_surface, xlabel="theta_1", ylabel="theta_0", zlabel="SSE", title="SSE Surface Plot")
# savefig("sse_surface_3D.png")